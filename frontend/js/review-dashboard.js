...
// Read a design token so chart colors follow the active theme (see tokens.css)
function chartToken(name) {
    return getComputedStyle(document.body).getPropertyValue(name).trim();
}

// Append a hex alpha channel to a hex token value (e.g. '#E05843' -> '#E0584359')
function chartAlpha(hexToken, alpha) {
    return hexToken + Math.round(alpha * 255).toString(16).padStart(2, '0');
}
...
function updateMasteryChart(distribution) {
    ...
    const data = {
        ...
        datasets: [{
            ...
            // Single-hue vermillion ramp: New (neutral) -> Expert (full accent)
            backgroundColor: (() => {
                const accent = chartToken('--accent');
                return [
                    chartToken('--text-3'),
                    chartAlpha(accent, 0.35),
                    chartAlpha(accent, 0.55),
                    chartAlpha(accent, 0.75),
                    accent,
                    chartToken('--accent-hover')
                ];
            })(),
            borderWidth: 2,
            borderColor: chartToken('--surface')
        }]
    };
...
function updateSourceChart(bySource) {
    ...
    const data = {
        ...
        datasets: [{
            ...
            backgroundColor: (() => {
                const accent = chartToken('--accent');
                return [accent, chartAlpha(accent, 0.55), chartToken('--text-3')];
            })(),
            borderWidth: 1,
            borderColor: chartToken('--surface')
        }]
...
async function refreshDashboard() {
    ...
    if (refreshBtn) {
        ...
        refreshBtn.innerHTML = '<svg class="icon spinner"><use href="icons.svg#refresh"/></svg> Refreshing...';
    }
    ...
    try {
        ...
        if (refreshBtn) {
            ...
            refreshBtn.innerHTML = '<svg class="icon"><use href="icons.svg#refresh"/></svg> Refresh';
        }
    }
}
